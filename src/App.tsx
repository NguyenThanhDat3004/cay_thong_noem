...
const PhotoOrnaments = ({
  ...
  useFrame((stateObj, delta) => {
    ...
    // Kiểm tra xem có vừa mới giơ ngón trỏ lên không
    const justRaisedFinger = isIndexUp && !wasIndexUpRef.current;
    const justLoweredFinger = !isIndexUp && wasIndexUpRef.current;

    // Nếu vừa giơ ngón trỏ lên, tìm ảnh gần nhất và KHÓA nó
    if (justRaisedFinger && state === "CHAOS") {
      let closestIndex = -1;
      ...
      // KHÓA ảnh MỚI này lại (mỗi lần giơ ngón trỏ sẽ tìm ảnh mới)
      lockedIndexRef.current = closestIndex;
      console.log("🔒 Locked NEW photo index:", closestIndex);
    }

    // Nếu vừa hạ ngón trỏ xuống, MỞ KHÓA để lần sau tìm ảnh MỚI
    if (justLoweredFinger) {
      console.log("🔓 Unlocked photo index:", lockedIndexRef.current);
      lockedIndexRef.current = -1;
    }

    // Cập nhật trạng thái trước đó
    wasIndexUpRef.current = isIndexUp;

    // Sử dụng ảnh đã khóa thay vì tìm lại
    hoverIndexRef.current = lockedIndexRef.current;

    // Apply position, scale và rotation
    groupRef.current.children.forEach((group, i) => {
      const objData = data[i];

      if (isIndexUp && state === "CHAOS" && hoverIndexRef.current !== -1) {
        if (i === hoverIndexRef.current) {
          const centerWorldPos = new THREE.Vector3(0, 2, 0);
          const targetScale = 18;
          const targetQuat = new THREE.Quaternion();
          camera.getWorldQuaternion(targetQuat);

          // Di chuyển ra trung tâm
          objData.currentPos.lerp(centerWorldPos, delta * 1.5);
          group.position.copy(objData.currentPos);

          // Scale nhanh mà vẫn smooth
          const scaleSpeed = 6; // tăng tốc độ scale
          group.scale.set(
            MathUtils.damp(group.scale.x, targetScale, scaleSpeed, delta),
            MathUtils.damp(group.scale.y, targetScale, scaleSpeed, delta),
            MathUtils.damp(group.scale.z, targetScale, scaleSpeed, delta)
          );

          // Quay theo camera mượt
          group.quaternion.slerp(targetQuat, delta * 5);
        } else {
          // Các ảnh khác scale nhỏ lại mượt
          const baseScale = objData.scale * zoom * 0.5;
          group.scale.set(
            MathUtils.damp(group.scale.x, baseScale, 3, delta),
            MathUtils.damp(group.scale.y, baseScale, 3, delta),
            MathUtils.damp(group.scale.z, baseScale, 3, delta)
          );
        }
      } else {
        // ===== CHẾ ĐỘ BÌNH THƯỜNG =====
        ...
        // 2️⃣ Scale - SMOOTH với easing
        const baseScale = objData.scale * zoom;
        const currentScale = group.scale.x;
        const scaleDiff = Math.abs(baseScale - currentScale);
        const scaleSpeed = scaleDiff > 2 ? delta * 3 : delta * 2;
        group.scale.lerp(
  ...
  return (
    <group ref={groupRef}>
      {data.map((obj, i) => (
        <group
          key={i}
          scale={[obj.scale * zoom, obj.scale * zoom, obj.scale * zoom]}
...
const Experience = ({
  ...
  return (
    <>
      <PerspectiveCamera makeDefault position={[0, 8, 60 / zoom]} fov={45} />
...
const GestureController = ({
  ...
  const isIndexUpRef = useRef<boolean>(false);
        ...
        if (videoRef.current.videoWidth > 0) {
          ...
          if (results.gestures.length > 0) {
            ...
            if (results.landmarks.length > 0) {
              ...
              const onlyIndexUp =
                indexUp && middleDown && ringDown && pinkyDown;

              isIndexUpRef.current = onlyIndexUp;

              // Gửi trạng thái ngón trỏ ra ngoài
              if (onIndexUp) {
                onIndexUp(onlyIndexUp);
              }

              // Xử lý gesture với score cao
              if (score > 0.4) {
              ...
              const targetZoom = onlyIndexUp ? 1.6 : 1; // zoom in/out target
              zoomRef.current = MathUtils.damp(
                zoomRef.current, // current
                targetZoom, // target
                4, // damping factor (càng cao càng mượt)
                1 / 60 // delta time approximation ~1 frame
              );
              onZoom(zoomRef.current);

              if (onlyIndexUp) {
                onMove(0);
                if (debugMode) onStatus("INDEX FINGER UP - ZOOMING");
              } else {
                const centerX = 0.5;
                const rawX = lm[0].x;
                const fixedX = 1 - rawX;
                const deltaX = fixedX - centerX;
                const speed = deltaX * 0.25;
                onMove(Math.abs(speed) > 0.01 ? speed : 0);
              }

              // Gửi vị trí ngón trỏ ra ngoài
              if (onPointer) {
                onPointer({ x: indexTip.x, y: indexTip.y });
              }
            }
          } else {
            onMove(0);
            if (onIndexUp) onIndexUp(false);
  ...
  }, [onGesture, onMove, onStatus, debugMode, onPointer, onIndexUp]);
